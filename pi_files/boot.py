...
fileName = "/home/pi/Edge_of_Space/datalogger_ACM0_" + time_stamp + ".csv" #name of the file you are going to write to
fileName2 = "/home/pi/Edge_of_Space/datalogger_ACM1_" + time_stamp + ".csv" # File for the second Arduino port
...
while result is None:
    try:
        ...
        result = serial.Serial(arduino_port1, baud)
    except:
        pass
    

ser_0 = serial.Serial(arduino_port0, baud)
ser_1 = serial.Serial(arduino_port1,baud)
print("Attempting to connecting to Arduino with Port:  " + arduino_port0)
print("Attempting to connect to Arduino with Port:  " + arduino_port1)
...
while True:
    ...
    getData=str(ser_1.readline())
    data=getData[0:][:-2]
    print(data)
    
    with open(fileName2, "a") as file_1:
        time_stamp = strftime('%Y-%m-%d-%H_%M_%S', gmtime())
        file_1.write(time_stamp + "," + data + "\n")
